...
object KafkaReadZipkinStatusSplitter {
  //General
  ...
  def main(args: Array[String]) {
    ...
    val spanStream = getSpanStreamFromKafka(ssc)
...
////     spanTraceStreamMaxDurationSpanLogs.print(50)
    ...
    val statusCodes = Set("200","500", "404")
// @transient    val statusCodeStreams = statuCodes.map(key => key -> spanTagStream.filter(x => x._3.get("http.status_code").getOrElse("").asInstanceOf[String].equals(key)))

    spanTagStream.foreachRDD(rdd => processRDD(rdd, statusCodes))
...
//    val found = statusCodeStreams.find(p => p._1.equals("200")).getOrElse(null)
//    
//    found._2.foreachRDD(rdd =>
...
//
//                        
//                        val message=new ProducerRecord[String, String](found._1,null,errorJSON)
...
//) 
  ...
  private def createSparkConf(): SparkConf = { 
      ...
      val sparkConf = new SparkConf().setAppName(sparkAppName).setMaster(sparkMaster).set("spark.local.dir", sparkLocalDir).registerKryoClasses(Array(classOf[Span])).registerKryoClasses(Array(classOf[Spans])).registerKryoClasses(Array(classOf[java.util.Map[String, String]]))
      sparkConf
  }
  
  private def getSpanStreamFromKafka(ssc: StreamingContext) = {
...
//    json.print(1000)
  ...
  private def processRDD(rdd: RDD[(Host, Span, Map[String, String])], statusCodes: Set[String]) = {
    
    rdd.cache()
    
    val statusRDDs = statusCodes.map(key => key -> rdd.filter(x => x._3.get("http.status_code").getOrElse("").asInstanceOf[String].equals(key)))
    
    for(statusRdd <- statusRDDs){
      statusRdd._2.foreachPartition(
          partitionOfRecords =>
            {
                processPartitionOfRecords(partitionOfRecords, statusRdd._1)
          })
    }
  ...
  private def processPartitionOfRecords(partitionOfRecords:  Iterator[(Host, Span, Map[String, String])], topic: String) = {
    ...
    val props = new HashMap[String, Object]()
                ...
                partitionOfRecords.foreach
                {
                     x=>{
                        ...
                        val message=new ProducerRecord[String, String](topic,null,errorJSON)
                        producer.send(message)
                        println(errorJSON)
                    }
                }
